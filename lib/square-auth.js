// lib/square-auth.js
// Simplified version for Rabbit Hole - just reads from Supabase
...
/**
 * Get Square auth credentials for Rabbit Hole
 */
export async function getSquareAuth() {
  try {
    const clientId = process.env.RABBIT_HOLE_CLIENT_ID;
    
    if (!clientId) {
      throw new Error('RABBIT_HOLE_CLIENT_ID not configured');
    }
...
/**
 * Get location ID for Rabbit Hole
 */
